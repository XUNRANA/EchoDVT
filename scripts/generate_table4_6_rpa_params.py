...
RTF_OUTPUT = ROOT / "results" / "table4_6_rpa_params.rtf"
TSV_OUTPUT = ROOT / "results" / "table4_6_rpa_params.tsv"
...
def build_rows():
    return [
        ("参数名", "默认值", "说明"),
        ("max_drift", "0.15", "允许的最大漂移距离，按图像对角线归一化"),
        ("min_good_frames", "3", "建立基线偏移所需的最少好帧数"),
        ("min_area", "100", "忽略面积过小的掩码，避免噪声干扰"),
    ]
...
def build_rtf(rows):
    ...
    lines = [
        ...
        r"\viewkind4\uc1",
        r"\pard\qc\f0\fs21\b " + rtf_escape("表4.6 RPA参数配置") + r"\b0\par",
        r"\pard\par",
    ]
