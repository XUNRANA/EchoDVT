...
OUTPUT_PATH = ROOT / "artifacts/figures/fig5_2.png"
...
def build_figure():
    ...
    fig.text(
        0.10,
        0.94,
        "验证集共 76 例（Normal 38，DVT 38），采用阈值 0.05 的高灵敏度判定；格内百分比按每个真实类别内占比计算。",
        ha="left",
        va="center",
        fontsize=11.0,
        color=TEXT,
    ...
    for r in range(2):
        for c in range(2):
            ...
            rect = Rectangle((c, r), cell_w, cell_h, facecolor=face, edgecolor="white", linewidth=3.0)
            ax.add_patch(rect)
    ...
    ax.set_xticklabels(["Normal", "DVT"], fontsize=13, color=TEXT, fontproperties=font)
    ax.set_yticks([0.5, 1.5])
    ax.set_yticklabels(["Normal", "DVT"], fontsize=13, color=TEXT, fontproperties=font)
