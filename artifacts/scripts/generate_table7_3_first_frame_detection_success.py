...
RTF_OUTPUT = ROOT / "artifacts" / "tables" / "table7_3_first_frame_detection_success.rtf"
TSV_OUTPUT = ROOT / "artifacts" / "tables" / "table7_3_first_frame_detection_success.tsv"
...
def build_rows():
    return [
        ("划分", "病例数", "Artery成功率(%)", "Vein成功率(%)", "两类同时成功率(%)", "Artery mIoU", "Vein mIoU"),
        ("train", "300", "100.0%", "100.0%", "100.0%", "0.9303", "0.9330"),
        ("val", "76", "90.8%", "90.8%", "85.5%", "0.7788", "0.7759"),
    ]


def build_rtf(rows):
    cellx = [1400, 2600, 4900, 7200, 9700, 11750, 13800]
    lines = [
        r"{\rtf1\ansi\deff0",
        r"{\fonttbl{\f0\fnil\fcharset134 SimSun;}{\f1\fnil Times New Roman;}}",
        r"\viewkind4\uc1",
        r"\pard\qc\f0\fs21\b " + rtf_escape("表7.3 首帧病例检测成功率与平均IoU") + r"\b0\par",
        r"\pard\par",
    ]
    ...
    for idx, row in enumerate(rows):
        ...
        aligns = [r"\qc", r"\qc", r"\qc", r"\qc", r"\qc", r"\qc", r"\qc"]
